...
public class KafkaUtilAll {
    ...
    public static List<String> partitionsTopic(String topic) throws ExecutionException, InterruptedException {
        ...
        AdminClient client = KafkaAdminClient.create(props);
        // 获取 topic 列表
        List<String> descGroups = client.describeTopics(Arrays.asList(topic)).all().get().get(topic).partitions()
                .stream()
                .flatMap(partition -> Stream.of(String.valueOf(partition.partition())+":"+String.valueOf(partition.replicas().size())))
    ...
    public static List<String> descCluster() throws ExecutionException, InterruptedException {
        ...
        AdminClient client = KafkaAdminClient.create(props);
        // 获取 topic 列表
        List<String> descGroups = client.describeCluster().nodes().get()
                .stream()
                .flatMap(node -> Stream.of(node.host()+":"+String.valueOf(node.port()))).collect(Collectors.toList());
        System.out.println("=======================");
        System.out.println("descGroups  : " + descGroups );
        //System.out.println("descGroups1  : " + descGroups1 );
        System.out.println("=======================");
    ...
    //消费者列表,值得注意的是，上面这个函数无法获取非运行中的consumer group，
    // 即虽然一个group订阅了某topic，但是若它所有的consumer成员都关闭的话这个函数是不会返回该group的。
    ...
    // 依旧要受Kafka的“一个Partition只能被该Group里的一个Consumer线程消费”规则的限制
    ...
    public static KafkaProducer<String, String> getNewKafkaProducer() throws ExecutionException, InterruptedException {
        ...
        //生产消息-ACK机制。0-意味着producer不等待broker同步完成的确认，继续发送下一条(批)信息；
        // 1-意味着producer要等待leader成功收到数据并得到确认，才发送下一条message；
    ...
    private static LinkedHashMap<String, Object> consumerPositions(String topic, String groupId) throws ExecutionException, InterruptedException {
        ...
        for (PartitionInfo partitionInfo : partitionInfos) {
            ...
            try {
                ...
                long current = kafkaConsumer.committed(topicPartition).offset();
//                long current = kafkaConsumer.endOffsets(partitions).get(topicPartition).offset();
